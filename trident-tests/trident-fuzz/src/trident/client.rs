...
impl Trident {
    ...
    /// Registers extra keypairs used to sign transactions (e.g. mint for InitializeStablecoin).
    /// Signers are used in addition to the payer when building the signature set.
    pub fn set_extra_signers(&mut self, signers: Vec<solana_sdk::signature::Keypair>) {
        self.extra_signers = signers;
    }

    /// Clears any extra signers previously set.
    pub fn clear_extra_signers(&mut self) {
        self.extra_signers.clear();
    }
    ...
    fn process_instructions(
        ...
        if num_required > 0 && num_required <= tx.message.account_keys.len() {
            let signer_pubkeys = &tx.message.account_keys[0..num_required];
            let mut signers: Vec<solana_sdk::signature::Keypair> = Vec::with_capacity(num_required);
            for pk in signer_pubkeys {
                if *pk == payer.pubkey() {
                    signers.push(payer.insecure_clone());
                } else if let Some(kp) = self.extra_signers.iter().find(|k| k.pubkey() == *pk) {
                    signers.push(kp.insecure_clone());
                } else {
                    // Missing keypair for this signer; skip signing and let the SVM report the error
                    break;
                }
            }
            if signers.len() == num_required {
                let blockhash = Hash::default();
                let refs: Vec<&solana_sdk::signature::Keypair> = signers.iter().collect();
                let _ = tx.try_sign(&refs, blockhash);
            }
        }
